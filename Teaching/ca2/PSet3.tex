...
\begin{enumerate}[label=\arabic*), itemsep=-0.1em]
		...
		\item Given $R$-module homomorphisms $A \xrightarrow{f} B \xrightarrow{g} C$ with $gf=0$, is exact if and only if 
		\[A \xrightarrow{f} B \xrightarrow{g} F \otimes_R C \text{ is exact if and only if } F \otimes_R A \xrightarrow{1 \otimes f} F \otimes_R B \xrightarrow{1 \otimes g} F \otimes_R C \text{ is exact.} \]
	\end{enumerate}
\end{problem}
