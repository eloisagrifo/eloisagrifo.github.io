...
\item Fix a proper ideal $I$, $n \geqslant 1$, and let $M$ be an $R$-module such that $\Ext^i_R(R/I,M) =0$ for all $i \neq n$.
Show that if
\[ \xymatrix{
 0 \arrow[r] &	E^0 \arrow[r]^-{\partial^0} & E^1 \arrow[r] & \cdots
}\]
is an injective resolution for $M$, then
\[ \xymatrix@C=1.3em{
 0 \arrow[r] & \coker (\partial^{n-1}_*) \arrow[rr]^-{\partial^n_*} && \Hom_R(R/I, E^{n+1}) \arrow[rr]^-{\partial^{n+1}_*} &&  \Hom_R(R/I, E^{n+2}) \arrow[rr]^-{\partial^{n+2}_*} && \cdots
}\]
is an injective resolution for $\Ext^n_R(R/I,M)$, where $\partial^i_* = \Hom_R(R/I, \partial^i)$.
\end{enumerate}
\end{problem}
