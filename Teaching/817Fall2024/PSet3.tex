...
\item\label{order of pairs} Show that if $|g|$ and $|h|$ are both finite, then $|(g,h)| = \lcm(|g|,|h|)$.
...
be the group whose elements are sequences of integers, equipped with the operation given by componentwise addition. Let $H=(\ZZ,+)$. Show that $G\times H \cong G$.
\end{problem}
